...
```{r}
# Reaction time
processed_rt <- read_tsv(file.path(current_version, "data", "Processed", "aspp_rt_processed.tsv"))
processed_rt_exp <- read_tsv(file.path(current_version, "data", "Processed", "aspp_valence_rt_nmin3_processed.tsv"))
...
cse_plot_rt_data <- 
  ...
  mutate(word_congruency_group = case_when(is_congruent == "Congruent" & is_prev_cong == 1 ~ 0,
                                           is_congruent == "Incongruent" & is_prev_cong == 1 ~ 1,
                                           is_congruent == "Congruent" & is_prev_cong == 0 ~ 2,
                                           is_congruent == "Incongruent" & is_prev_cong == 0 ~ 3,)) %>% 
...
#Save the figure
```{r}
ggsave(file.path(current_version, "prime_probe_valence_rt_cse.png"), width = 14.4, height = 8, plot = last_plot())
```
...
rt_random_intercep_model_interaction <- lmer(rt ~  is_congruent * is_prev_cong + (1|filename), data = processed_rt) # mindenkinek az interceptje máshol van. 

interaction_effect <- summary(rt_random_intercep_model_interaction)
interaction_effect
```
...
Bf(sd = interaction_effect$coefficients[6],
   ...
   meanoftheory = 0,
   sdtheory = 73,#27938,#0.09, # 73ms is the congruency effect obtained in Weissman et al (2014) on prime probe
   dftheory = 10^10,
   tail = 1)
