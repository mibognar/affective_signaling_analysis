...
processed_rt_nmin3 <- read_tsv(file.path(current_version, "data", "Processed", "aspp_arousal_rt_processed_nmin3.tsv"))
```
...
#Save the figure
```{r}
ggsave(file.path(current_version, "prime_probe_arousal_rt_cse.png"), width = 14.4, height = 8, plot = last_plot())
```
...
rt_random_intercept_model_interaction <- lmer(rt ~  is_congruent * is_prev_cong + (1|filename), data = processed_rt) # mindenkinek az interceptje máshol van. 

interaction_effect_summary<- summary(rt_random_intercept_model_interaction)
...
Bf(sd = interaction_effect_summary$coefficients[8],
   ...
   meanoftheory = 0,
   sdtheory = 36.5,#4.09,#1000,#36.5, # half of the congruency effect obtained in Weissman et al (2014)
   dftheory = 10^10,
   tail = 1)
...
Bf(sd = interaction_effect_summary$coefficients[6],
   ...
   meanoftheory = 0,
   sdtheory = 73,#27231,#0.07,#73, # 73ms is the congruency effect obtained in Weissman et al (2014) on prime probe
   dftheory = 10^10,
   tail = 1)
