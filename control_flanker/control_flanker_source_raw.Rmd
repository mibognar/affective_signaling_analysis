...
## Merging participant and demographic data
```{r}
...
participant_id = bind_rows(participant_id, participant_id2)
demographics = bind_rows(demographics, demographics2)
...
test_participants <- 
  participant_id %>% 
  filter(user_id %in% c("v","66","666666","k","100","99"))
...
participant_id<-
  participant_id %>% 
  filter(user_id %ni%  c("v","66","666666","k","100","99"))
...
participant_id <-
  participant_id %>% 
  left_join(., select(participant_duplicate, user_id,id, duplicate_drop), by = c("id","user_id"))
```
...
# count how many participants are kept
```{r}
source %>% 
...
```{r save tasks}
write_tsv(source, file.path(current_version, "data", "Raw", "flanker_raw_data.tsv"))
```
