...
# Accuracy analysis
```{r}
acc_random_intercept_slope_model <- glmer(as.factor(is_correct) ~ as.factor(is_prev_congruent) * as.factor(is_congruent) + (1|filename), data = processed_acc, family = binomial)

accuracy_model = summary(acc_random_intercept_slope_model)
accuracy_model
```
```{r}
Bf(sd = accuracy_model$coefficients[8],
   obtained = accuracy_model$coefficients[4],
   dfdata = 180802,
   meanoftheory = 0,
   sdtheory =.1,
   dftheory = 10^10,
...
```{r}
acc_predicted = processed_acc %>% 
  mutate(predicted_acc = predict(acc_random_intercept_slope_model, type = "response"))
```
...
cse_plot_acc <- 
  ...
  geom_errorbar(aes(ymin = mean_acc - se_acc,
                ...
                width = .1) +
  scale_shape_manual(values = c(4, 16)) +
  scale_y_continuous(limits = c(0.94, 0.99)) +
  scale_x_discrete(expand = c(1, 0)) +
  xlab("Congruency of the previous trial")+
  ylab("Mean accuracy") +
  guides(shape = guide_legend(title = "Congruency of \n the current trial")) +
...
```
```{r}
ggsave(file.path(current_version, "figures", "flanker_acc_cse.png"), width = 14.4, height = 8, plot = last_plot())
```
