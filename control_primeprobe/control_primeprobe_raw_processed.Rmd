...
myfile = osf_retrieve_node("v2sn4") %>%
  osf_ls_files() %>% 
  filter(name == "prime_probe_global_raw_data.tsv")

osf_download(myfile, raw_folder)

raw = read_tsv(file.path(raw_folder, "prime_probe_global_raw_data.tsv"))
...
```{r}
raw <- read_tsv(file.path(current_version, "Data", "Raw", "prime_probe_global_raw_data.tsv"))
```
...
# Check how many participants has to be excluded on accuracy

```{r calculate accuracy}
test %>% group_by(participant_id, drop_participant_acc) %>% count() %>% group_by(drop_participant_acc) %>% count()
```
...
rt_all_missing %>% group_by(drop_all_missing_rt) %>% count()
...
# Check how many participants have to be excluded based on rt criteria
```{r}
test %>% group_by(participant_id, drop_participant_rt) %>% count() %>% group_by(drop_participant_rt) %>% count()
...
test <-
  ...
  mutate(age_drop = case_when(age < 18 ~ 1L,
                              age > 99 ~ 1L,
                              TRUE ~ 0L))
test %>% group_by(participant_id, age_drop) %>% count() %>% group_by(age_drop) %>% count()
...
test %>% group_by(participant_id, drop_participant_acc, drop_participant_rt) %>% count() %>% group_by(drop_participant_acc, drop_participant_rt) %>% count()
```
...
```{r}
#hardcoded proportion of dropped kept trials
...
write_tsv(response_rt_analysis, file.path(current_version, "Data", "Processed", "prime_probe_global_processed_rt.tsv"))

# Saving data for the accuracy analysis
write_tsv(response_acc_analysis, file.path(current_version, "Data", "Processed", "prime_probe_global_acc.tsv"))
