...
```{r}
processed_cohen <- read_tsv(file.path(current_version, "data", "Processed", "flanker_arousal_cohen_effect_analysis.tsv"))
```


```{r}
only_cong = processed_cohen %>% 
  filter(n_min3_cong == 1)

onlycong_model = lmer(rt ~ is_prev_cong + (1+is_congruent|filename), data = only_cong)
cohen2_model = lmer(rt ~ n_min3_cong * is_prev_cong + (1+is_congruent|filename), data = processed_cohen)
...
```{r}
mypredicted = processed_cohen %>% 
  mutate(predicted_rt=predict(cohen2_model))
...
processed_threeway <- read_tsv(file.path(current_version, "data", "Processed", "flanker_arousal_threeway_interaction_analysis.tsv"))

threeway_model = lmer(rt ~ is_congruent * n_min3_cong * n_min4_arousal + (1+is_congruent|filename), data = processed_threeway)
...
```{r}
acc_processed = read_tsv(file.path(current_version, "data", "Processed", "flanker_arousal_processed_acc.tsv"))
...
is_prev_cong = case_when(is_prev_cong ==  0L ~ "High arousal",
 ...
 is_prev_cong = fct_relevel(is_prev_cong, c("Low arousal", "Hight arousal")),)%>%
