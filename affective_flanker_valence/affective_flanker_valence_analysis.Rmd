...
sequential_cse_model = lmer(rt ~ is_congruent * n_min3_cong + (1+is_congruent|participant_id), sequential_cse_data)
...
Bf(sd = sequential_cse_summary$coefficients[8],
   ...
   meanoftheory = 0,
   sdtheory = 26.5,#11656,#1.1,#26.5, #needs RR
   dftheory = 10^10,
   tail = 1)
```
...
sequential_cse_plot
ggsave(file.path(current_version,"figures/sequential_cse_plot.png"), width = 14.4, height = 8, plot = last_plot())
...
# Cohen effect
cohen_effect_data = read_tsv(file.path(current_version, "data", "Processed", "flanker_valence_cohen_effect.tsv"))


cohen2_model = lmer(rt ~ n_min3_cong * is_prev_cong + (1+is_congruent|filename), data = cohen_effect_data)
cohen_summary = summary(cohen2_model)
...
```{r}
# Threeway effect
threeway_effect_data = read_tsv(file.path(current_version, "data", "Processed", "flanker_valence_threeway_interaction.tsv"))
```



```{r}
threeway_model = lmer(rt ~ is_congruent * n_min3_cong * n_min4_valence + (1+is_congruent|filename), data = threeway_effect_data)
...
Bf(sd = threeway_summary$coefficients[16],
   ...
   meanoftheory = 0,
   sdtheory = 10,#7.4,#70.6, RRB>3 = [7.4, 70.6]
   dftheory = 10^10,
   tail = 1)
...
cse_plot_3way_nmin4 <- 
  ...
  scale_y_continuous(limits = c(750, 1200)) +
  scale_x_discrete(expand = c(1, 0)) +
  xlab("Congruency of the previous trial (n-3)")+
  ylab("Reaction time") +
  guides(shape = guide_legend(title = "Congruency \n of the current trial")) +
  papaja::theme_apa() +
  theme(legend.position = c(0.5,0.25),
...
```{r}
ggsave(file.path(current_version,"figures/cse_plot_3way_nmin4.png"), width = 10.8, height = 6, plot = cse_plot_3way_nmin4)
```
...
```{r}
acc_processed = read_tsv(file.path(current_version, "data", "Processed", "flanker_valence_processed_acc.tsv"))
...
Bf(sd = affective_summary$coefficients[7],
   ...
   dfdata = affective_summary$coefficients[11],
   meanoftheory = 0,
   sdtheory = 0.2,
   dftheory = 10^10,
   tail = 1)
