...
# Figures of CSE
## Exploratory plot for nmin3 analysis

```{r}
cse_plot_rt_data <- 
  processed_rt_nmin3 %>% 
  
  mutate(is_nmin3_cong = case_when(is_nmin3_cong ==  0L ~ "Incongruent",
                                  is_nmin3_cong ==  1L ~ "Congruent"),
         is_nmin3_cong = as_factor(is_nmin3_cong),
         is_nmin3_cong = fct_relevel(is_nmin3_cong, c("Congruent", "Incongruent")),
         is_congruent = case_when(is_congruent ==  0L ~ "Incongruent",
                                 is_congruent ==  1L ~ "Congruent"),
         is_prev_cong = case_when(is_prev_cong == 0 ~ "Negative",
                                  is_prev_cong == 1 ~ "Neutral")) %>%
  group_by(participant_id, is_nmin3_cong, is_congruent, is_prev_cong) %>% 
  summarise(participant_mean_rt = mean(rt, na.rm = T)) %>% 
  mutate(word_congruency_group = case_when(is_congruent == "Congruent" & is_prev_cong == "Neutral" ~ 0,
                                           is_congruent == "Incongruent" & is_prev_cong == "Neutral" ~ 1,
                                           is_congruent == "Congruent" & is_prev_cong == "Negative" ~ 2,
                                           is_congruent == "Incongruent" & is_prev_cong == "Negative" ~ 3,)) %>% 
  group_by(is_nmin3_cong, is_congruent, is_prev_cong, word_congruency_group) %>% 
  summarise(N = n(),
...
cse_plot_rt <- 
  ...
  xlab("Congruency of the nmin3 trial")+
  ylab("Reaction time") +
...
cse_plot_rt_data_nmin4 <- 
  ...
  mutate(is_nmin3_cong = case_when(is_nmin3_cong ==  0L ~ "Incongruent",
         ...
         is_congruent = case_when(is_congruent ==  0L ~ "Incongruent",
                                 is_congruent ==  1L ~ "Congruent"),
         n_min4_valence = case_when(n_min4_valence == 0 ~ "Negative",
                                  n_min4_valence == 1 ~ "Neutral")) %>%
  group_by(participant_id, is_nmin3_cong, is_congruent, n_min4_valence) %>% 
  summarise(participant_mean_rt = mean(rt, na.rm = T)) %>% 
  mutate(word_congruency_group = case_when(is_congruent == "Congruent" & n_min4_valence == "Neutral" ~ 0,
                                           is_congruent == "Incongruent" & n_min4_valence == "Neutral" ~ 1,
                                           is_congruent == "Congruent" & n_min4_valence == "Negative" ~ 2,
                                           is_congruent == "Incongruent" & n_min4_valence == "Negative" ~ 3,)) %>% 
  group_by(is_nmin3_cong, is_congruent, n_min4_valence, word_congruency_group) %>% 
  summarise(N = n(),
            mean_rt = mean(participant_mean_rt, na.rm = T),
            sd_rt = sd(participant_mean_rt, na.rm = T),
            se_rt = sd_rt / sqrt(N))
