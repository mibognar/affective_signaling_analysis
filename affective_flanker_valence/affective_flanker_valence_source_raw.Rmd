...
rerun_data_pth <-file.path(current_version, "data", "rerun")
...
rerun_source <- read_plus(pattern = ".csv$",
                    path = rerun_data_pth,
                    include = "2021") %>% 
  janitor::clean_names()
...
demographics_rerun <- read_csv(file.path(current_version, "Data","rerun", "demographics.csv" )) %>% 
  janitor::clean_names()
...
participant_id_rerun <- read_csv(file.path(current_version, "Private","rerun", "participant-metadata.csv")) %>% 
  janitor::clean_names()
```

# merging first and rerun data

```{r}
participant_id_rerun = participant_id_rerun %>% 
  mutate(id = id +5000)
demographics_rerun = demographics_rerun %>% 
  mutate(id = id + 5000)
rerun_source = rerun_source %>% 
  mutate(id = id + 5000)


participant_id = bind_rows(participant_id, participant_id_rerun)
demographics = bind_rows(demographics, demographics_rerun)
source = bind_rows(source,rerun_source)
```
...
```{r save tasks}
write_tsv(source, file.path(current_version, "data", "Raw", "flanker_valence_raw_data.tsv"))
```
